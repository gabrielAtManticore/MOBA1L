...
function Tick(deltaTime)
	...
	if currentState == STATE_ENGAGING then
		...
		elseif IsWithinRangeSquared(target, ATTACK_RANGE_SQUARED) and attackCooldown <= 0 then
...
function CanHear(noisePos)
	...
	if (distSquared < HEARING_RADIUS_SQUARED) then
		return true
	end
	return false
end
...
function CanSeeEnemy(enemy, myPos, forwardVector, nearestDistSquared)
	...
	local canSeeFromDistance = (distSquared <= VISION_RADIUS_SQUARED)
	if (not canSeeFromDistance and
		currentState == STATE_LOOKING_AROUND and
		distSquared < SEARCH_RADIUS_SQUARED and
		SEARCH_RADIUS_SQUARED > VISION_RADIUS_SQUARED) then
		
		local p = (distSquared - VISION_RADIUS_SQUARED) / (SEARCH_RADIUS_SQUARED - VISION_RADIUS_SQUARED)
	...
	if (canSeeFromDistance and distSquared < nearestDistSquared) then
		if (VISION_HALF_ANGLE <= 0 or VISION_HALF_ANGLE >= 360) then
		...
		if (angle <= VISION_HALF_ANGLE) then
			return true, distSquared
		end
	end
	--print("dist = " .. tostring(math.sqrt(distSquared)) .. ", " .. tostring(distSquared) .. ", " .. tostring(VISION_RADIUS_SQUARED))
	return false
end
